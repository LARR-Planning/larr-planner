...
def get_min_dist_index(arr, x, y):
    # print("arr: ", arr)
    # print(type(arr))
    # print(arr.shape)
    new1 = np.sqrt(np.power((arr[0][:] - x), 2) + np.power((arr[1][:] - y), 2))
    val = np.amin(new1)
    ind = np.argmin(new1)
    pos_xs = []
    pos_ys = []
...
class MotionPlanner(Node):
    ...
    def timer_callback(self):
        ...
        if self.global_flag == 1:
            ...
            [dist, closest_ind_in] = get_min_dist_index(self.map_path, self.position_x, self.position_y)
