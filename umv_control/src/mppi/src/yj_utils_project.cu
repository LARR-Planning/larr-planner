...
const float a_max = 1.0f;
const float a_min = -2.0f;
...
void getCostParallel(const std::vector<float *> &choosed_obs_traj_ss, const std::vector<float *> &choosed_obs_traj_eys,
                                ...
                                float *cuda_As, 
                                float *cuda_Ss, float * cuda_next_Ss, float *cuda_first_ss, float *pre_ss, float *cuda_lat_T) {
    ////////////////
    // total_cost //
    ////////////////
    float ratio = 1;
...
void feasibleTrajSamplingParallel(float *ref_ss, float *ref_yaws, float *curvs, 
    ...
    cudaDeviceSynchronize();
        ...
        // float *eyss = new float[sampling_cnt];
        // float *dss = new float[sampling_cnt];
        // cudaDeviceSynchronize();
        ...
        // cudaMemcpy(eyss, d_eys, sizeof(float) * sampling_cnt, cudaMemcpyDeviceToHost); 
        // cudaMemcpy(dss, d_dss, sizeof(float) * sampling_cnt, cudaMemcpyDeviceToHost); 
    ...
    // cudaMemcpy(traj_eyaws, d_traj_eyaws, sizeof(float)*sampling_cnt_N0, cudaMemcpyDeviceToHost);
    // cudaMemcpy(traj_ss, d_traj_ss, sizeof(float)*sampling_cnt_N0, cudaMemcpyDeviceToHost);
    // cudaMemcpy(traj_dvs, d_traj_dvs, sizeof(float)*sampling_cnt_N0, cudaMemcpyDeviceToHost);
    // cudaDeviceSynchronize();
    ...
    d_traj_ss, d_traj_eys, d_traj_eyaws, d_traj_dvs, d_As, d_Ss, d_next_steers, d_first_steers, d_pre_steers, d_lat_cost);
